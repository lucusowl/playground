...
아래의 환경에서 동작을 확인했음을 알림, 참고바람

HW side         |SW side
----------------|----------------
Raspberry Pi 4B |Ubuntu Server - 22.04.4

## 작업 요약

1. [(선택) 시스템 정보 확인](#1-선택-시스템-정보-확인)
2. [관리자 계정 활성화](#2-관리자-계정-활성화)
3. [네트워크 설정](#3-네트워크-설정)
4. [패키지 업데이트](#4-패키지-업데이트)
5. [시스템 시간 동기화](#5-시스템-시간-동기화)
6. [(선택) SSH 원격 연결 활성화](#6-선택-ssh-원격-연결-활성화)
7. [(선택) 기타 편의 설정](#7-선택-기타-편의-설정)
   1. 키보드 레이아웃
   2. 시스템 언어 & 로케일
   3. 스토리지
   4. 응용 계층 서비스
